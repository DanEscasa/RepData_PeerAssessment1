...
However, the more interesting pattern to look at is `intervalSteps[c(12, 13),]` or, in general, `intervalSteps[c(x, x + 1), ]` where `x >= 12`. Note that `intervalSteps[c(x, x + 1), ]$interval = (i1, i2)`, and `i1` is congruent to 55 mod 100, i2 is congruent to 0 mod 100.
...
`R -e "rmarkdown::render('PA1_template.Rmd')"` no matter your OS. You can also render it using the `Knit` button (where's the fun in that) or the R console, with the command `rmarkdown::render('PA1_template.Rmd')`.

<<<<<<< HEAD
||||||| merged common ancestors
Oddly, activity from 10:00 AM all the way to 5:30 PM is heavier on weekdays, then wane at 11:00 PM for both weekends and weekdays.
=======
Oddly, activity from 10:00 AM all the way to 5:30 PM is heavier on weekends, then wane at 11:00 PM for both weekends and weekdays.
>>>>>>> f617e3e72bd45b6fa8156492eb587faf1fd57f26
