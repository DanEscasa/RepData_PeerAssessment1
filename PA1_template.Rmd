...
```{r echo = TRUE}
stepsStats <- merge(totalDaySteps, meanDaySteps, by = "date")
stepsStats <- merge(stepsStats, medianDaySteps, by = "date")
colnames(stepsStats) <- c("date", "totalSteps", "meanSteps", "medianSteps")
```
...
```
Plotting mean of number of steps per day.
```{r echo = TRUE}
with(stepsStats, 
     barplot(meanSteps, names.arg = date))
```
Lastly, plot the median number of steps per day.
```{r echo = TRUE}
with(stepsStats, 
     barplot(medianSteps, names.arg = date))
```
