...
## Install ggplot2 and knitr if necessary
...
As an aside, take note of the use of the superassignment (`<<-`) instead of the usual assignment operator (`<-`). This is because the `with(){}` creates its own scope. And yes, the scope applies to the `colnames()` function as well.
```{r echo = TRUE}
with(data = activity,{
...
Median is zero because of the large number of zeroes in that column. `NA`s may also account for that.

And here's the dataframe of median number of steps per day:
```{r}
kable(dayStats[, c(1, 4)],
      col.names = c("date", "median"),
      format.args = list(big.mark = ","))
```
## What is the average daily activity pattern?
